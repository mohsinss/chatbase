...
(function () {
    ...
    async function initializeChatElements() {
        ...
        tooltip.innerHTML = "Click here to chat with us!";
        ...
        async function toggleChatWindow() {
            ...
            console.log(chatContainer.style.display === 'none')
            if (chatContainer.style.display === 'none' || chatContainer.style.display === '') {
        ...
        document.addEventListener('click', function (event) {
            if (!chatContainer.contains(event.target) && !chatIcon.contains(event.target) && !tooltip.contains(event.target)) {
                chatContainer.style.display = 'none';
                if (!settings?.chatIconUrl) {
                    chatIcon.innerHTML = '<div>💬</div>';
                }
        ...
        const chatbotIcon = document.getElementById("chatbot-widget-icon");
        if (!chatbotIcon) return;

        // Tooltip positioning logic
        const positionTooltip = () => {
            const iconRect = chatbotIcon.getBoundingClientRect();
            tooltip.style.left = `${iconRect.left - 20}px`;
