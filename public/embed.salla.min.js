(function() {
    ...
    const chatbotStyles = `
        ...
        /* Suggested messages improvements */
        ...
        .suggested-messages-container::-webkit-scrollbar {
          display: none;
        }
        ...
        .suggested-message-button:hover {
          background-color: #e5e7eb;
        }
        
        .dark-theme .suggested-message-button {
          background-color: #374151;
          color: white;
        }
        
        .dark-theme .suggested-message-button:hover {
          background-color: #4b5563;
        }
        
        .suggested-messages-fade-left,
        .suggested-messages-fade-right {
          position: absolute;
          top: 0;
          bottom: 0;
          width: 32px;
        ...
        .dark-theme .suggested-messages-fade-left {
          background: linear-gradient(to right, rgba(17,24,39,1), rgba(17,24,39,0));
        }
        
        .dark-theme .suggested-messages-fade-right {
    ...
    async function fetchChatbotSettings(playgroundChatbotId) {
        try {
    ...
    //Fetch chatbotId
    async function fetchChatbotId(storeId) {
        try {
            const response = await fetch(`${window.location.protocol}//${window.embeddedChatbotConfig.domain}/api/chatbot/integrations/salla/getChatbotId?merchant=${storeId}`);
            const data = await response.json();
            return data.chatbotId;
        } catch (error) {
            console.error('Error fetching chatbot settings:', error);
            return null;
        }
    ...
    async function initializeChatElements() {
        ...
        const storeId = window?.embeddedChatbotConfig?.store_id || 'defaultStoreId';
        const playgroundChatbotId = await fetchChatbotId(storeId);
        console.log('Playground Chatbot ID:', playgroundChatbotId);

        const settings = await fetchChatbotSettings(playgroundChatbotId);
        ...
        if (settings?.chatIconUrl) {
            ...
            chatIcon.innerHTML = '<div>💬</div>'; // Default emoji
        }
        ...
        tooltipContent.innerHTML = settings?.initialMessage ?? "Psst... Wanna know the ultimate secret to 3x revenue growth? 💰";
        ...
        const positionTooltip = () => {
            ...
            // Position tooltip closer to the chat icon - reduced gap from 15px to 8px
            tooltip.style.left = `${iconRect.left - tooltip.offsetWidth - 8}px`;
            ...
            if (window.innerWidth <= 640) {
                ...
                // For mobile, position arrow at bottom center for a speech bubble effect
                ...
                tooltipAfter.textContent = `
                    #chatbot-tooltip::after {
                        ...
                        border-color: ${tooltip.style.backgroundColor === 'black' ? 'black' : 'white'} transparent transparent transparent;
                    }
                `;
                document.head.appendChild(tooltipAfter);
            } else {
                // Position tooltip closer to the chat icon - reduced gap from 15px to 8px
                tooltip.style.left = `${iconRect.left - tooltipRect.width - 8}px`;
                tooltip.style.top = `${iconRect.top + iconRect.height / 2}px`;
                tooltip.style.transform = "translateY(-50%)";
                
                // Reset arrow to default right position
                const tooltipAfter = document.createElement('style');
                tooltipAfter.textContent = `
                    #chatbot-tooltip::after {
                        right: -10px;
                        ...
                        transform: translateY(-50%);
                        border-width: 10px 0 10px 10px;
                        border-color: transparent transparent transparent ${tooltip.style.backgroundColor === 'black' ? 'black' : 'white'};
                    }
                `;
                document.head.appendChild(tooltipAfter);
            }
        ...
        // Ensure accurate initial positioning
        const ensureProperTooltipPosition = () => {
            // Initial position to avoid flash of unpositioned content
            positionTooltip();
            
            // Re-position after a short delay to get accurate measurements
            setTimeout(positionTooltip, 50);
            
            // Re-position again after all content is fully loaded
        ...
        if (settings?.tooltipDelay !== -1) { // Don't show if set to "never"
            ...
            setTimeout(() => {
                ...
                // Run positioning after display is set and with increased reliability
                ensureProperTooltipPosition();
        ...
        // Reposition tooltip on window resize with debounce to improve performance
        let resizeTimer;
        ...
        function toggleChatWindow(e) {
            ...
            if (chatContainer.style.display === 'none' || chatContainer.style.display === '') {
                ...
                if (settings?.userMessageColor) {
                    ...
                    chatIcon.innerHTML = '<div style="color: white; font-size: 24px;">✕</div>';
                } else {
                    chatIcon.innerHTML = '<div style="font-size: 24px;">✕</div>';
                }
        ...
        document.addEventListener('click', function(e) {
            ...
            if (chatContainer.style.display === 'flex') {
                // Check if click is outside chat container and chat icon
                if (!chatContainer.contains(e.target) && e.target !== chatIcon && !chatIcon.contains(e.target)) {
                    toggleChatWindow();
                }
            }
        });
        ...
        window.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'chatbot-settings-update') {
                ...
                if (tooltip && document.body.contains(tooltip)) {
                    ...
                    // Update content
                    tooltipContent.innerHTML = updatedSettings.initialMessage || "Psst... Wanna know the ultimate secret to 3x revenue growth? 💰";
                    ...
                    if (!document.getElementById('chatbot-tooltip')) {
                        setTimeout(() => {
                            ...
                            newTooltipContent.innerHTML = updatedSettings.initialMessage || "Psst... Wanna know the ultimate secret to 3x revenue growth? 💰";

                            // Close button
                            const newTooltipClose = document.createElement("div");
                            newTooltipClose.id = "chatbot-tooltip-close";
                            newTooltipClose.innerHTML = "✕";
