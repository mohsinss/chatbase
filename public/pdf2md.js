...
const getTaskPages = async (taskId, taskIdToDisplay) => {
    ...
    try {
        ...
        console.error(e);
        notyf.error(`Error fetching task pages. Please try again later. ${e}`);
    }
};
...
copyButton.addEventListener("click", (e) => {
    ...
    notyf.success(`Copied ${jsonSwitch.classList.contains("bg-purple-600") ? "JSON" : "Markdown"
            } to clipboard!`);
});
...
fileUploadInput.addEventListener("change", (event) => {
    ...
    reader.onload = (event) => {
        ...
        fetch("https://pdf2md.trieve.ai/api/task", {
            ...
            .then((data) => {
                notyf.success("File uploaded! We are processing the file. Please wait. Scroll down to the table to view the status.");
            ...
            .catch((error) => {
                notyf.error(`Error uploading file. Please try again later. ${error}`);
            });
    };
