    ...
    <div className="p-0 h-full">
      ...
      <div className="flex flex-col gap-6 p-6 bg-[#F7F9F9] mt-[120px]">
        ...
        <div className="bg-gray-50 p-6 rounded-lg">
          <h1 className="text-2xl font-semibold text-gray-900">LinkedIn Reactions</h1>
          <p className="mt-2 text-gray-600">Manage your LinkedIn chatbot reactions and settings.</p>
          ...
          <div className="flex flex-col gap-6 mt-6">
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Share DMs</h3>
              <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-200">
                  <div className="flex items-center gap-3">
                    <Switch
                      checked={settingsData.shareDmEnabled}
                      onChange={(enabled) => setSettingsData({ ...settingsData, shareDmEnabled: enabled })}
                      className={`${settingsData.shareDmEnabled ? 'bg-blue-600' : 'bg-gray-200'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
                    >
                      <span className={`${settingsData.shareDmEnabled ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
                    </Switch>
                    <span className="text-sm font-medium text-gray-700">Send DM After Post Share</span>
                  </div>
                </div>

                <div className={`space-y-6 ${!settingsData.shareDmEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
                  <div className="bg-white p-4 rounded-lg border border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <Switch
                          checked={settingsData.shareDmFirstOnly}
                          onChange={(enabled) => setSettingsData({ ...settingsData, shareDmFirstOnly: enabled })}
                          className={`${settingsData.shareDmFirstOnly ? 'bg-blue-600' : 'bg-gray-200'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
                        />
                        <span className="text-sm font-medium text-gray-700">Send DM Only on First Share</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-lg border border-gray-200">
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Default Share DM Prompt</label>
                        <textarea
                          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3 focus:ring-blue-500 focus:border-blue-500"
                          value={settingsData.shareDmPrompt}
                          onChange={(e) => setSettingsData({ ...settingsData, shareDmPrompt: e.target.value })}
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Default Delay (seconds)</label>
                        <input
                          type="number"
                          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500"
                          value={settingsData.shareDmDelay}
                          onChange={(e) => setSettingsData({ ...settingsData, shareDmDelay: Number(e.target.value) })}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-lg border border-gray-200">
                    <h4 className="text-sm font-medium text-gray-700 mb-4">Specific Post Settings</h4>
                    <div className="space-y-4">
                      {(settingsData.shareDmSpecificPosts || []).map((post, index) => (
                        <div key={index} className="flex flex-col gap-3 p-4 bg-gray-50 rounded-lg">
                          <div>
                            <label className="block text-sm font-medium text-gray-700">Post URL</label>
                            <input
                              ...
                              value={post.postUrl}
                              onChange={(e) => {
                                const newPosts = [...(settingsData.shareDmSpecificPosts || [])];
                                newPosts[index].postUrl = e.target.value;
                                setSettingsData({ ...settingsData, shareDmSpecificPosts: newPosts });
                              }}
                            />
                          </div>
                          <div>
                      ...
                      <button
                        onClick={() => {
                          const newPosts = [...(settingsData.shareDmSpecificPosts || []), { postUrl: '', prompt: '', delay: undefined }];
                          setSettingsData({ ...settingsData, shareDmSpecificPosts: newPosts });
                        }}
                        className="text-sm font-medium text-blue-500 hover:text-blue-700"
                      >
